...
const validateAndUploadMiddleware = asyncErrorHandler(
  async (req, res, next) => {
  ...
  `;
    ...
    if (requiredDocuments.length === 0) {
      return res
        .status(400)
        .json({
          msg: "No required documents found for this user",
          success: false,
        });
    }

    const requiredDocs = requiredDocuments.map((doc) => doc.name);
    const uploadFields = requiredDocs.map((doc) => ({
      name: doc,
      maxCount: 1,
    }));
    ...
    uploadDocs(req, res, async (err) => {
      if (err) {
        ...
        return res
          .status(400)
          .json({ msg: "Error uploading files.", success: false });
      }
      ...
      if (!files || Object.keys(files).length === 0) {
        return res
          .status(400)
          .json({ msg: "No files uploaded.", success: false });
      }
      ...
      for (const doc of requiredDocuments) {
        ...
        if (doc.doc_ext === "image") {
          ...
          if (!allowedImageExtensions.includes(fileExtension)) {
            errors.push(
              `Invalid file type for ${doc.name}. Only jpeg, jpg, png are allowed.`
            );
            continue;
          }
        } else {
      ...
      if (errors.length > 0) {
        return res
          .status(400)
          .json({ msg: "Document validation failed", errors, success: false });
      }

      try {
        console.log(
          `Starting upload of files: ${Object.keys(files).join(", ")}.`
        );
            ...
            const uploadStream = cloudinary.uploader.upload_stream(
              ...
              (error, result) => {
                if (error) {
                  ...
                  console.log(
                    `File ${key} uploaded successfully: ${result.secure_url}`
                  );
        ...
        res
          .status(500)
          .json({ msg: "Upload error", error: error.message, success: false });
      }
    });
  }
);

...
const NewApplications = () => {
  ...
  const { newBuyers } = useSelector((state) => state.buyers);
  // PAGINATION CODE
  const usersPerPage = 10;
  const indexOfLastUser = currentPage * usersPerPage;
  const indexOfFirstUser = indexOfLastUser - usersPerPage;
  const currentUsers =
    newBuyers && newBuyers.slice(indexOfFirstUser, indexOfLastUser);
  const totalPages = Math.ceil(newBuyers && newBuyers.length / usersPerPage);
  ...
  const getAllNewBuyers = async () => {
    setLoading(true);
    const data = await callApiGet("get-all-pending-buyers");
    console.log(data);
    if (data.success) {
      if (data.data) {
        dispatch(addAllNewBuyers(data.data));
      } else {
        dispatch(addAllNewBuyers([]));
      }
      setLoading(false);
    } else {
  ...
  return (
    <div className="w-full" style={{ padding: "16px" }}>
      <HeaderTitle
        title={"New Applications Of Buyers"}
      ...
      <div className="flex flex-wrap justify-between gap-4 pt-4 pb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            onClick={() => router.push("/buyers/add-buyer")}

...
import { addAllRejectedBuyers } from "@/store/slices/buyersSlice";
...
const RejectedApplication = () => {
  ...
  const { rejectedBuyers } = useSelector((state) => state.buyers);
  // PAGINATION CODE
  const usersPerPage = 10;
  const indexOfLastUser = currentPage * usersPerPage;
  const indexOfFirstUser = indexOfLastUser - usersPerPage;
  const currentUsers =
    rejectedBuyers && rejectedBuyers.slice(indexOfFirstUser, indexOfLastUser);
  const totalPages = Math.ceil(
    rejectedBuyers && rejectedBuyers.length / usersPerPage
  );
  ...
  return (
    <div className="w-full" style={{ padding: "16px" }}>
      <HeaderTitle
        title={"Rejected Applications Of Buyers"}
